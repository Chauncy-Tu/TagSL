...
int dwt_initialise(uint16 config)
{
...

    {
        ...
        reg |= 0x00004000 ; //7 and 8 to mode - to be used with PA
		reg |= 0x00050000 ; //8 and 9 to mode - RX/TX testing
        dwt_write32bitreg(GPIO_CTRL_ID,reg);

//		dwt_setGPIOdirection(GxM5, 0);
//		dwt_setGPIOvalue(GxM5, GDP5);
		
//		dwt_setGPIOdirection(GxM6, 0);
//		dwt_setGPIOvalue(GxM6, GDP6);
...
static u8 disable_crc_flag = 0;
void disable_crc_error_handling(void)
{

	u32 sysCFGregTmp;
	sysCFGregTmp = dwt_read32bitreg(SYS_CFG_ID);

	/////////////////////////////
	sysCFGregTmp |= SYS_CFG_DIS_FCE ;
	//////////////////////
	dwt_write32bitreg(SYS_CFG_ID,sysCFGregTmp) ;
	disable_crc_flag = 1;
...
//    configure PLL2/RF PLL block CFG
    ...
    {
        dwt_xtaltrim(tag_configdata.UWB_XTRIM); //3	
    }
...
int dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset)
{
	...
	if(disable_crc_flag && tag_configdata.SELF_POSITION != ENABLE_SELF_POSITION_MODE ) 
		txFrameLength = txFrameLength -2;
...
uint32 _dwt_otpsetmrregs(int mode)
{
    ...
    // Wait?
    userPlatformApi.user_delay_ms(10);
    // Set Clear Mode sel
    wr_buf[0] = 0x00;
    dwt_writetodevice(OTP_IF_ID,OTP_CTRL+1,1,wr_buf);
    ...
    wr_buf[0] = 0x04;
    dwt_writetodevice(OTP_IF_ID,OTP_CTRL+1,1,wr_buf);
    userPlatformApi.user_delay_ms(100);
...
//      printf("OTP PROG 32: ERROR VPP NOT OK, programming will fail. Are MR/MRA/MRB set?\n");
    ...
    while(otp_done == 0)
    {
		userPlatformApi.user_delay_ms(1);
        dwt_readfromdevice(OTP_IF_ID, OTP_STAT, 1, rd_buf);
...
int dwt_spicswakeup(uint8 *buff, uint16 length)
{
    if(dwt_readdevid() != DWT_DEVICE_ID) //device was in deep sleep (the first read fails)
    {
        ...
        userPlatformApi.user_delay_ms(5);
    }
    else
    {
        return DWT_SUCCESS;
...
{
    uint8 x = (tag_configdata.NTM | (tag_configdata.PMULT << 5)); //LDE_PARAM1;
...
int _dwt_loaducodefromrom(void)
{
	...
	userPlatformApi.user_delay_ms(1);; // Allow time for code to upload (should take up to 120 us)
...
void dwt_isr(void) // assume interrupt can supply context
{
    ...
    if(tag_configdata.PRINT_UWB_MSG_MDOE != 0)
        if(signalPowerGlobal.dw1000StatusNumbes < DW1000_MAX_STATUS)
            signalPowerGlobal.dw1000StatusHex[signalPowerGlobal.dw1000StatusNumbes++] = status;
    ...
    //
    if((status & SYS_STATUS_RXFCG) || (disable_crc_flag && (status & SYS_STATUS_RXFCE))) 
	{
		if(status & SYS_STATUS_LDEDONE)  // LDE done/finished
		{
			...
			else //no overrun condition - proceed to process the frame
			{
				...
				if(dw1000local.dblbuffon == 0)  //如果不是doublebuffer 就读长度
				{
                ...
                if(dw1000local.sysCFGreg&SYS_CFG_AUTOACK)
                {	
...
//                    dwt_write32bitreg(SYS_STATUS_ID,clear) ;         // write status register to clear event bits we have seen
                    ...
                    buff = dw1000local.cdata.status>>24; 	 //不再次读取，直接使用一开始读的status，不知道会不会有问题
					...
					if(auto_correction_flag == 1)
					{
						msgCarrierGloble = dwt_readcarrierintegrator();
					}
...
int dwt_rxenable(uint8 mode)
{
    ...
    {
        temp1 =dwt_read16bitoffsetreg(SYS_STATUS_ID,3) ;// Read 1 byte at offset 3 to get the 4th byte out of 5
        if ((temp1 & (SYS_STATUS_HPDWARN >> 24)) != 0) // if delay has passed do immediate RX on unless DWT_IDLE_ON_DLY_ERR is true
        {
            dwt_forcetrxoff(); // turn the delayed receive off

            if((mode & 2) == 0) // if DWT_IDLE_ON_DLY_ERR not set then re-enable receiver
            {
...
uint16 dwt_readtempvbat(uint8 fastSPI)
{
    ...
    if(fastSPI == 1)
    {
    	userPlatformApi.user_delay_ms(1); //if using PLL clocks(and fast SPI rate) then this nrf_delay_ms is needed
...
void UWB_Rx_On(uint16 rxTimeout)
{
	u8 temp = (uint8)SYS_CTRL_TRXOFF;
	dwt_write32bitreg(SYS_STATUS_ID,0xFFFFFFFF);
	dwt_writetodevice(SYS_CTRL_ID,0,1,&temp);                  //?????????
	dwt_setrxtimeout(rxTimeout); //reconfigure the timeout
	dwt_rxenable(DWT_START_RX_IMMEDIATE);
	if(tag_configdata.TOF_POSITION_MODE == NEW_TOF_POSITION_MODE)
	{
		uwbUserStatus.uwbHeadRxFlag = 1;
	}
}
...
/*
	????dw1000?????
*/
void UWB_Rx_OFF(void)
{
	u8 temp = (uint8)SYS_CTRL_TRXOFF;

	dwt_write32bitreg(SYS_STATUS_ID,0xFFF7FFFF);
	dwt_writetodevice(SYS_CTRL_ID,0,1,&temp);                  //?????????
}
